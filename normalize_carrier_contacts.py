...
def main():
    ...
    contacts = session.execute(
        text("SELECT id, company_tax_type, company_tax_number, company_name FROM platform.carriers_contacts")
    ).fetchall()
    ...
    for row in contacts:
        ...
        if to_update:
            ...
            session.execute(
                text(f"UPDATE platform.carriers_contacts SET {set_clause} WHERE id = :cid"),
                params
            )
